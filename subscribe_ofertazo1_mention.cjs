...
async function main() {
  ...
  console.log('Suscribiendo mention y feed para Ofertazo1...');
  const res = await callGraph(`/${pageId}/subscribed_apps?subscribed_fields=feed,mention&access_token=${page.access_token}`, 'POST');
  console.log('Resultado Suscripción:', JSON.stringify(res.data));
}
