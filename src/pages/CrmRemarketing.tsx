import { RefreshCcw, Clock, AlertCircle, FileText, CheckCircle2, XCircle, Search, Filter } from 'lucide-react';

export function CrmRemarketing() {
  const columns = [
    {
      id: 'cooling',
      title: 'Enfriados (< 24h)',
      ...
      tooltip: 'Permite mensajes de texto libre.'
    },
    {
      id: 'contact_1',
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
            <RefreshCcw className="w-6 h-6 text-blue-600" /> CRM Remarketing (Carritos Abandonados)
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            Rescata ventas caídas de ShopyEasy. <span className="font-semibold text-orange-600">Regla Meta:</span> Más de 24h = Plantilla Obligatoria.
          </p>
        </div>
        ...
        {columns.map((column) => (
          <div key={column.id} className="flex-shrink-0 w-80 flex flex-col snap-center">
            ...
            <div className={`flex-1 p-3 border-x border-b rounded-b-xl ${column.color} flex flex-col gap-3 overflow-y-auto`}>
              ...
              {/* Dummy Cards for < 24h */}
              {column.id === 'cooling' && (
                <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-200 cursor-pointer hover:border-blue-400 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded uppercase">Visto hace 2h</span>
                    <span className="text-xs font-semibold text-gray-400">ShopyEasy</span>
                  </div>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    <button className="flex-1 text-xs font-bold bg-blue-50 text-blue-700 py-1.5 rounded-lg hover:bg-blue-100">Escribir (Libre)</button>
                  </div>
                </div>
              )}
