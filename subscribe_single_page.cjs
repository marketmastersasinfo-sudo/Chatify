...
async function main() {
  const pageId = '409267838930893'; // Distribuidor Oficial Paula Velez
  const { data: page } = await supabase.from('connected_pages').select('*').eq('page_id', pageId).single();

  console.log(`===========================================================`);
  console.log(`ACTIVANDO WEBHOOK PARA: ${page ? page.page_name : pageId}`);
  console.log(`===========================================================\n`);

  const res = await callGraph(`/${pageId}/subscribed_apps?subscribed_fields=feed&access_token=${page.access_token}`, 'POST');
  console.log('Resultado Suscripción Webhook Feed:', JSON.stringify(res.data));
}
